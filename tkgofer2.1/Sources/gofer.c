...
#if TK_GOFER
    int tk_debug  = 0;                 /* toggle for displaying tk scripts */
    extern int c_quit_tcl ();
#endif
...
static Bool quitGofer = FALSE;          /* TRUE => gofer already quitted   */
...
#if TK_GOFER
    {'x', "Show TkGofer debug information",        &tk_debug},
#endif
...
String argv[]; {
#if SIGJMP
    Int errorNumber = sigsetjmp(catch_error,1);
#else
