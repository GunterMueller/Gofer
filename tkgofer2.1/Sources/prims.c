...
static String local evalName2		Args((Cell));
#endif
#if IO_DIALOGUE
...
/* prototype definition for tkgofer primitives */
#if TK_GOFER 

static Int    local evalSpineOfList     Args((Cell));
static String local evalString          Args((Cell));
static List   local buildString         Args((String));

PROTO_PRIM(primInitTcl);
PROTO_PRIM(primRunTcl);
PROTO_PRIM(primExecuteTcl);
PROTO_PRIM(primGetTcl);
PROTO_PRIM(primSetVar);
#endif
...
/* Reference definition for tkgofer primitives */
#if TK_GOFER
  {"primInitTcl",	1, NoGofcPrim(primInitTcl)},
  {"primRunTcl",	1, NoGofcPrim(primRunTcl)},
  {"primGetTcl",	1, NoGofcPrim(primGetTcl)},
  {"primExecuteTcl",	2, NoGofcPrim(primExecuteTcl)},
  {"primSetVar",	2, NoGofcPrim(primSetVar)},
#endif
/* ------------------------------------------- */
...
static String local evalName2(es)        /* evaluate es :: [Char] and save  */
Cell es; {                              /* in char array... return ptr to  */
    static char buffer[TK_MAX+1]; /* string or 0, if error occurs    */
    Int         pos    = 0;
    StackPtr    saveSp = sp;
...
primFun(primIOPutchar) {		/* print character on stdout	   */
    ...
    putchar(myc);
/*    myCounter++;
    if ((myCounter % 1000) == 0) {putchar('.');}
    switch (myc) {
    case 'E': mydfaState=1;
              break; 
    case 'O': if (mydfaState==1) mydfaState++; else mydfaState=0;
              break;
    case 'M': if (mydfaState==2) mydfaState++; else mydfaState=0;
              break;
    default:  mydfaState=0;
              break;          
    }
    if (mydfaState==3) putchar(':'); 
*/     
    if (myc=='\n') fflush(stdout);
    updapRoot(ap(mkTuple(2),UNIT),s);
}
...
primFun(primSetVar) {   		/* put string in tcl variable      */
                       		  	/*  :: String ->                   */
    Cell   s = primArg(1);		/*	IO ()			   */
    String c = evalName2(primArg(2));   /*  eval variable contents         */
...
primFun(primExecuteTcl) {   		/* evaluate string in tk           */
    Cell es = primArg(2);		/*  :: String ->		   */
    Cell s  = primArg(1);		/*	IO String  */
    char *inp;
    char *out;
    eval(s);

    inp = evalName2(es);
    out = c_execute_tcl(inp);

    updapRoot(ap(mkTuple(2),buildString(out)),s);
}

primFun(primGetTcl) {			/* get character from event buffer */
    Cell s = primArg(1);		/*  :: IO Char			   */
    eval(s);
    updapRoot(ap(mkTuple(2),mkChar(c_get_tcl())),s);
}

primFun(primInitTcl) {			/* initialize tcl/tk               */
    Cell s = primArg(1);		/*  :: IO Bool	         	   */
    eval(s);
    updapRoot(ap(mkTuple(2),(c_init_tcl() ? nameTrue : nameFalse)),s);
}
...
/* --------------------------------------------------------------------------
 * A couple of functions to convert between Gofer datatypes and C datatypes
 * ------------------------------------------------------------------------*/

/* Evaluate spine of a list, return its length.
   The elements IN the list are untouched...
*/

static Int local evalSpineOfList(es)
Cell es;
{
    int length = 0;
    ...
    eval(es);
    while( whnfHead==nameCons ) {
      pop();                    /* leave head unevaluated */
      length++;
      eval( pop() );            /* evaluate tail */
    }                           /* now, whnfHead == nameNil */
    return length;
}

/* Evaluate a list of characters.
   Save all characters in heap-allocated storage.
*/

static String local evalString(es)
Cell es; {
    char *string;
    int str_len;
    int count = 0;

    /* first, evaluate the spine of the list to figure out how much storage
       is needed.
    */

    str_len = evalSpineOfList( es );
    string = malloc( str_len * sizeof(char) + 1 );

    /* now, evaluate all characters in the list */

    unwind(es);                 /* spine of list is already evaluated */
    while( whnfHead==nameCons ) {
      eval( pop() );                    /* head, needs evaluation */
      string[count++] = charOf( whnfHead );
      unwind( pop() );                  /* tail, already evaluated... */
    }
    string[count] = '\0';
    return string;
}

static List local buildString(s)        /* build a string from a character */
String s; {                             /* array                           */
    List vs = NIL;
    for (; *s; s++)
        vs = ap(ap(nameCons,mkChar(*s)),vs);
    return revOnto(vs,nameNil);
}
