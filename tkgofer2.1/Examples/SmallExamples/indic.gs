...
type IndState = Clipboard (Int,Int,Int)               -- value, width, height
...
indicator cs w =
  ...
  do c <- canvas [] w
     ...
     i <- crect (0,0) (0,0) [] c
     j <- crect (0,0) (0,0) [] c
     st <- clipboard [ initValue (0,20,100) ]
...
instance Widget Indicator where
  cset w c =
    ...
    where
      newheight ind v =
        do (i,x,y) <- getValue (stateI ind)
           setValue (stateI ind) (i,x,v)
...
instance HasInput WItem Indicator0 Int where
  getValue w = do (i,_,_) <- getValue (stateI w)
                  result i
 
  setValue w i =
    do (v,x,y) <- getValue (stateI w)
       setValue (stateI w) (i,x,y)
